...
if [ "$TRAVIS_TAG" ]; then
    ...
    sshpass -p $DEPLOY_PASS ssh $DEPLOY_USER@$DEPLOY_HOST  "cd $DEPLOY_PATH && tar -xvf release.tgz && rm -rf SEAP_PIP/.git && cat $TRAVIS_TAG > SEAP_PIP/build/release.txt && rm release.tgz && bash deploy-seap-pip.sh"

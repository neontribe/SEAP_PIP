...
if [ "$TRAVIS_TAG" ]; then
    ...
    sshpass -p $DEPLOY_PASS ssh $DEPLOY_USER@$DEPLOY_HOST  "cd $DEPLOY_PATH && tar -xvf release.tgz && mv www www-previous && rm -r SEAP_PIP/.git && mv SEAP_PIP www && rm release.tgz"
